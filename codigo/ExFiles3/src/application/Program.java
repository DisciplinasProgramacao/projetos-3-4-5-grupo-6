	...
	public static void main(String[] args) {		
		...
		try (BufferedReader br = new BufferedReader(new FileReader(sourceFileStr))) {
			...
			while(itemCsv != null) {
				...
				list.add(new Vehicle(name, capacidadeTanque, valorVenda, kmTotal, kmDiario, consumo,  new Routes(null, 300.0, "Belo horizonte", "Rio de janeiro")));
			...
			try (BufferedWriter bw = new BufferedWriter(new FileWriter(targetFileStr))) {
				for(Vehicle item : list) {
					bw.write("Relatorio: " + item.getModelo() + ", " + String.format("%.2f", item.calculaIPVA()) + ", " + item.autonomia());
					bw.newLine();
				}
